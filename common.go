...
// 解压整个 ZIP 文件到目标目录
func unzipAll(zipFile, targetDir string) error {
	r, err := zip.OpenReader(zipFile)
	if err != nil {
		logger.Errorln("无法打开 ZIP 文件:", err)
		return err
	...
	for _, file := range r.File {
		...
		if !strings.HasPrefix(filepath.Clean(destPath), filepath.Clean(targetDir)+string(os.PathSeparator)) {
			logger.Warnf("跳过路径穿越文件: %s", file.Name)
			continue
		}
		...
		newFile, err := os.OpenFile(destPath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, file.Mode())
		if err != nil {
			rc.Close()
		...
		newFile.Close()
		if err != nil {
			logger.Errorln("无法解压 ZIP 文件中的内容:", err)
			return err
		}
	}
	logger.Debug("ZIP 解压完成.")
	return nil
}
...
func sumFileSHA256(filePath string) string {
	file, err := os.Open(filePath)
	if err != nil {
		logger.Error("无法打开文件:", err)
		return ""
